...
The Beyou Network uses distributed peer-to-peer network technology to build a distributed,
...
The Beyou Network provides distributed services such as Auto-Proxy,
multi-layer anonymous network, IM, non-polluted DNS, even CDN and DDoS defense, etc.
The :ref:`stoken` with stable value scale will be used in the ecology and produce
an incentive effect. Everyone can participate in distributed network construction.
We believe the Beyou Network,
...
The user running the service node makes use of idle network resources to earn Stokens,
and the user or organization that needs the service uses Stokens exchange to settle
...
With the help of Beyou Network,
users participating in the ecology can easily run BEYOU node programs on computers,
...
The longer the time is online, the more Stokens can be obtained,
...
The contract is settled automatically, the node inside the wall pays the :ref:`stoken` as the demand side,
and the outside node uses the idle bandwidth as the service party to provide
the encrypted traffic-proxy service and automatically accept the :ref:`stoken` settlement,
...
mobile devices, or even just a RaspberryPI device running BEYOU node programs,
...
but writes the ratio of :ref:`stoken` to network traffic flow resource service
...
:ref:`stoken` will be strictly based on the ERC20 token standard,
...
The Beyou Network pioneered the definition of the **Proof of Traffic** consensus mechanism,
the BEYOU node has only a very low performance requirement for the device,
...
but also Stoken’s dollar price is more stable,
...
Therefore, 100 GB client traffic flow, anchored to 1 Stoken,
...
At the same time, we expect that the Stoken value scale
after ecological maturity is relatively stable
(that is, the legal currency price is relatively stable),
and the basic setting of 1 Stoken = 100 GB can ensure that
the market circulation price of Stoken gradually matures
